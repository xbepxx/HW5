...
{
    #region ЗАНЯТИЕ 4. ПЛАГИН "СОЗДАНИЕ МОДЕЛИ". ЧАСТЬ 2.
    [TransactionAttribute(TransactionMode.Manual)]
    public class CreationWindowPlugin : IExternalCommand
    {
        ...
        private void AddRoof(Document doc, Level level2, List<Wall> walls) //создаём крышу
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                LocationCurve curve = walls[i].Location as LocationCurve; //получаем привые
